<template>
  <div>
    <div class="toolbar">
      <el-button type="primary" :icon="Plus" @click="$emit('create')">新增工序</el-button>
    ...
    <el-table :data="steps" v-loading="loading" border>
      ...
      <el-table-column prop="photoRequired" label="拍照" width="80">
        <template #default="{ row }">
          <el-tag :type="row.photoRequired ? 'warning' : 'info'">{{ row.photoRequired ? '需要' : '否' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="remarkRequired" label="备注" width="80">
        <template #default="{ row }">
          <el-tag :type="row.remarkRequired ? 'warning' : 'info'">{{ row.remarkRequired ? '需要' : '否' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="mobileEnabled" label="移动端" width="90">
        <template #default="{ row }">
          <el-tag :type="row.mobileEnabled ? 'success' : 'info'">{{ row.mobileEnabled ? '允许' : '否' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="280" fixed="right">
        <template #default="{ row }">
          <div class="table-actions">
            <el-button size="small" :disabled="!row.enabled" @click="$emit('move-up', row)">上移</el-button>
            <el-button size="small" :disabled="!row.enabled" @click="$emit('move-down', row)">下移</el-button>
...
function saveManualOrder() {
  const activeSteps = props.steps.filter((step) => step.enabled && !step.deleted)
