  ...
  <section>
    <div class="page-header">
      ...
      <el-button
        type="primary"
        :icon="Plus"
        @click="router.push('/process/route-templates/new')"
      >
        新建
      </el-button>
    </div>
        ...
        <el-table-column label="操作" width="260" fixed="right">
          <template #default="{ row }">
            <div class="table-actions">
              <el-button
                size="small"
                @click="router.push(`/process/route-templates/${row.id}/edit`)"
              >
                编辑
              </el-button>
              ...
              <el-button size="small" type="danger" @click="removeRoute(row)"
                >删除</el-button
              >
            </div>
          </template>
...
const filteredRoutes = computed(() =>
  filterRouteTemplates(routes.value, filters),
);
const productTypeFilterOptions = computed(
  () =>
    Array.from(
      new Set(routes.value.map((item) => item.productType).filter(Boolean)),
    ) as string[],
);
